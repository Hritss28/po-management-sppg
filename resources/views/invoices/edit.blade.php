...
@section('content')
    <div class="fixed inset-0 z-50 overflow-y-auto bg-slate-900/45 p-3 backdrop-blur-sm sm:p-6">
        <form method="POST" action="{{ route('invoices.update-items', $order['id']) }}" class="mx-auto my-0 max-w-[1100px] overflow-hidden rounded-xl bg-slate-50 shadow-2xl sm:rounded-2xl" data-invoice-form>
            @csrf
            @method('PATCH')
            ...
            <header class="flex items-center justify-between border-b border-slate-200 bg-white px-4 py-3 sm:px-6">
                <div class="flex min-w-0 items-center gap-3">
                    ...
                    <div class="min-w-0">
                        <h1 class="truncate text-base font-black tracking-tight text-slate-950 sm:text-lg">Edit Invoice</h1>
                        <p class="text-[10px] font-bold uppercase tracking-wide text-slate-400">{{ $invoice->number }} · {{ $supplier['name'] }}</p>
                    </div>
                </div>
                <a href="{{ route('invoices.index', ['tab' => 'history']) }}" class="text-2xl leading-none text-slate-400 hover:text-slate-700">&times;</a>
            </header>
                ...
                <div class="grid grid-cols-1 gap-3 lg:grid-cols-12">
                    <div class="flex items-center justify-between rounded-xl border border-slate-200 bg-white p-4 shadow-sm lg:col-span-4">
                        ...
                        <div class="text-right">
                            ...
                            <p class="mt-0.5 text-lg font-black text-blue-600">Rp <span data-invoice-total>{{ number_format($invoice->total_amount, 0, ',', '.') }}</span></p>
                        </div>
                    </div>
                    ...
                    <div class="flex flex-col gap-2 lg:col-span-3">
                        <button type="submit" class="w-full rounded-lg bg-slate-900 px-4 py-2.5 text-xs font-bold uppercase tracking-wide text-white shadow-sm transition hover:bg-blue-600">Simpan Perubahan</button>
                        <a href="{{ route('invoices.index', ['tab' => 'history']) }}" class="w-full rounded-lg border border-slate-200 bg-white px-4 py-2 text-center text-xs font-bold uppercase tracking-wide text-slate-600 transition hover:bg-slate-50">Batal</a>
                    </div>
                </div>
                                    ...
                                    @foreach ($items as $item)
                                        ...
                                        <tr class="invoice-item-row hover:bg-slate-50/50">
                                            ...
                                            <td class="px-3 py-2">
                                                ...
                                                <input type="text" name="items[{{ $itemIndex }}][name]" list="invoice-stock-items-list" autocomplete="off" value="{{ $item['name'] }}" class="invoice-item-name w-full min-w-[160px] rounded-md border border-slate-200 bg-slate-50 px-2 py-1.5 text-xs font-semibold text-slate-800 outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-500/10" placeholder="Ketik / pilih barang...">
                                            </td>
                                            <td class="px-2 py-2">
                                                <input type="text" name="items[{{ $itemIndex }}][unit]" value="{{ strtoupper($item['unit']) }}" class="item-unit-hidden w-full rounded-md border border-slate-200 bg-slate-50 px-2 py-1.5 text-xs font-semibold uppercase text-slate-800 outline-none focus:border-blue-500">
                                            </td>
                                            <td class="px-2 py-2">
                                                <input name="items[{{ $itemIndex }}][qty]" type="number" min="0.01" step="0.01" value="{{ $item['qty'] }}" class="invoice-qty w-full rounded-md border border-slate-200 bg-slate-50 px-2 py-1.5 text-xs font-semibold text-slate-800 outline-none">
                                            </td>
                                            <td class="px-2 py-2">
                                                <div class="flex items-center rounded-md border border-slate-200 bg-slate-50 px-2 py-1.5">
                                                    ...
                                                    <input name="items[{{ $itemIndex }}][price]" type="text" inputmode="numeric" data-currency-input value="{{ $item['price'] }}" class="invoice-price min-w-0 flex-1 bg-transparent text-xs font-semibold text-slate-800 outline-none">
                                                </div>
                                            </td>
            ...
            const buildNewRow = (idx) => {
                return `<tr class="invoice-item-row hover:bg-blue-50/30">
                    ...
                    <td class="px-2 py-2">
                        <div class="flex items-center rounded-md border border-slate-200 bg-slate-50 px-2 py-1.5">
                            <span class="mr-1 text-[9px] font-bold text-slate-400">Rp</span>
                            <input name="items[${idx}][price]" type="text" inputmode="numeric" value="0" required class="invoice-price min-w-0 flex-1 bg-transparent text-xs font-semibold text-slate-800 outline-none">
                        </div>
                    </td>
                    <td class="px-2 py-2 text-right text-xs font-bold text-slate-900">Rp <span class="invoice-subtotal">0</span></td>
                    <td class="px-2 py-2 text-center">
