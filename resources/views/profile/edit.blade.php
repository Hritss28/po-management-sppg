...
@section('content')
    <section class="mx-auto max-w-3xl bg-slate-50 p-2 sm:p-8">
        @if (session('success'))
        ...
        <form method="POST" action="{{ route('profile.update') }}" class="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-md shadow-slate-200/70">
            ...
            <footer class="flex flex-col-reverse gap-3 border-t border-slate-100 bg-slate-50 px-5 py-5 sm:flex-row sm:items-center sm:justify-end sm:gap-4 sm:px-8">
                <a href="{{ route('dashboard') }}" class="text-sm font-black text-slate-600">Batal</a>
                <button type="submit" class="w-full rounded-lg bg-blue-600 px-8 py-3 text-sm font-black text-white shadow-lg shadow-blue-600/20 transition hover:bg-blue-700 sm:w-auto">
                    Simpan Profile
                </button>
            </footer>
        </form>
